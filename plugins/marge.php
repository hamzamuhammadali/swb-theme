...
function getMargin($search_string) {
	// Öffnen Sie die CSV-Datei zum Lesen
	$csv_file = fopen( wp_upload_dir()['path'] . '/csv/' . get_option( 'last_uploaded_csv' ), 'r' );

	// Iteriere durch die Zeilen der CSV-Datei
	while (($row_data = fgetcsv($csv_file, 0, ';')) !== false) {
		// Überprüfe, ob das Suchmuster im aktuellen Array vorhanden ist
		if (strpos(implode(';', $row_data), $search_string) !== false) {
			// Gib den letzten Wert des Arrays zurück
			return end($row_data);
		}
	}
