...
$pdf->Ln(1.2);
$pdf->Cell(1);
$pdf->MultiCell(90, 4.5, utf8_decode('Ihr persönlicher Berater: ' . $userName . ($userPhone ? '
Kontakt: ' . $userPhone : '') . ($userMail ? ' | ' . $userMail : '')));
$pdf->setTextColor(0, 0, 0);
$pdf->SetFillColor(8, 170, 255);
$pdf->setY($pdf->getY() - 10);
$pdf->Cell(0, 5, 'Planegg' . ', ' . date("d.m.Y"), 0, 0, 'R');
$pdf->Ln();
$pdf->setTextColor(0,0,0);
$pdf->Ln();
$pdf->Ln();
...
Gerne stehen wir Ihnen jederzeit mit Rat und Tat zur Seite und unterstützen Sie in der zügigen Planung, Errichtung und Installation Ihrer Wärmepumpe.
...
$pdf->Cell(180,5, 'Angebotsdetails', '', '', 'C');
$pdf->Ln(6);
$pdf->Ln();
$pdf->SetFont('Helvetica', '', 9);
...
$pdf->MultiCell(180, 4.5, utf8_decode('Leistungsübersicht der SWB Solar GmbH'), '', 'C');
$pdf->SetFont('Helvetica', '', 9);
...
if ($storage) {
    $storageArray = array($storageInformation['name'][0], $storageInformation['beschreibung'][0], $storageInformation['masse'][0], $storageInformation['installation'][0], $storageInformation['wartung'][0], $storageInformation['sicherheit'][0], $storageInformation['sicherheit2'][0], $storageInformation['garantie'][0], $storageInformation['garantie2'][0]);
...
if ($query->have_posts()) :
    ...
    $pdf->Ln();
    $pdf->Ln();
    $pdf->MultiCell(150, 5, $pdf->writeHTML(utf8_decode(get_option('zahlung'))), '', 'L');
    $pdf->SetXY(165, $pdf->getY() - 5);
    $pdf->MultiCell(30, 5, number_format($totalIncl * 0.9, 2, ',', '.') . utf8_decode(EUR), '', 'R');
...
if ($calculation === 'Ja') {
    ...
    if ($totalPower >= 10 && $storage) {
        ...
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 30, 4.5, 1, 'F');
        $pdf->setTextColor(255, 255, 255);
        $pdf->MultiCell(29.5, 5, utf8_decode(number_format(($yearPower - ($energy * 0.8)) * get_option('ockw10kw40'), 2, ',', '.') . EUR), '', 'R');
        $pdf->setTextColor(0, 0, 0);
    } else if ($storage) {
        ...
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 30, 4.5, 1, 'F');
        $pdf->setTextColor(255, 255, 255);
        $pdf->MultiCell(29.5, 5, utf8_decode(number_format(($yearPower - $energy70 - $energy30) * get_option('kw10'), 2, ',', '.') . EUR), '', 'R');
        $pdf->setTextColor(0, 0, 0);
    } else {
        $pdf->MultiCell(65, 5, utf8_decode('Bis 10 kW'), '', 'L');
        $pdf->SetXY(65, $pdf->getY() - 5);
        ...
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 30, 4.5, 1, 'F');
        $pdf->setTextColor(255, 255, 255);
        $pdf->MultiCell(29.5, 5, utf8_decode(number_format(($yearPower - ($energy * 0.8)) * get_option('ockw10'), 2, ',', '.') . EUR), '', 'R');
        $pdf->setTextColor(0, 0, 0);
    }
    ...
    if ($storageInformation['cloud'][0] === 'Ja' && $storage) {
        ...
        $pdf->setTextColor(0, 0, 0);
    } else if (empty($storageInformation['cloud'][0] === 'Ja')) {
        ...
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 30, 4.5, 1, 'F');
        $pdf->setTextColor(255, 255, 255);
        $pdf->MultiCell(29.5, 5, utf8_decode(number_format(($energy * 0.7) * $kwhPriceCalculated, 2, ',', '.') . EUR), '', 'R');
        $pdf->setTextColor(0, 0, 0);
    } else {
    ...
    if ($storage && $storageInformation['cloud'][0] === 'Ja') {
        ...
        $pdf->setTextColor(0, 0, 0);
    } else if($storageInformation['cloud'][0] != 'Ja') {
        $pdf->MultiCell(150, 5, utf8_decode('Einspeisevergütung + Ersparnis jährlich gesamt '), '', 'L');
        $pdf->SetXY(165, $pdf->getY() - 5);
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 30, 4.5, 1, 'F');
        $pdf->setTextColor(255, 255, 255);
        $pdf->MultiCell(29.5, 5, utf8_decode(number_format($savings + $savingsConsumption, 2, ',', '.') . EUR), 0, 'R');
        $pdf->setTextColor(0, 0, 0);
        $pdf->MultiCell(150, 5, utf8_decode('Einspeisevergütung + Ersparnis monatlich gesamt '), '', 'L');
        $pdf->SetXY(165, $pdf->getY() - 5);
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 30, 4.5, 1, 'F');
        $pdf->setTextColor(255, 255, 255);
        $pdf->MultiCell(29.5, 5, utf8_decode(number_format(($savings + $savingsConsumption) / 12, 2, ',', '.') . EUR), 0, 'R');
        $pdf->setTextColor(0, 0, 0);
    }else {
    ...
    if ($storage && $storageInformation['cloud'][0] === 'Ja') {
        ...
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 30, 4.5, 1, 'F');
        $pdf->setTextColor(255, 255, 255);
        $pdf->MultiCell(29.5, 5, utf8_decode(number_format($savings + $savingsConsumption, 2, ',', '.') . EUR), '', 'R');
        $pdf->setTextColor(0, 0, 0);
    } else {
    ...
    if ($storage && $storageInformation['cloud'][0] === 'Ja') {
        ...
        $pdf->RoundedRect($pdf->getX(), $pdf->getY(), 30, 4.5, 1, 'F');
        $pdf->setTextColor(255, 255, 255);
        $pdf->MultiCell(29.5, 5, number_format((($savings + $savingsConsumption - $cloudsizePrice * 12) * 20 * 1.5) / $totalExcl / 20 * 100, 2, ',', '.') . '%', '', 'R');
        $pdf->setTextColor(0, 0, 0);
    } else {
        $pdf->MultiCell(150, 5, utf8_decode('das entspricht einem Ertragsverhältnis zur Nettoinvestition von '), '', 'L');
        $pdf->SetXY(165, $pdf->getY() - 5);
...
// email stuff (change data below)
    if(!empty($byMail)) {
        $to = 'post@swb-solar.de';
    } else {
        $to = $email;
    }
    $from = $userMail;
    $message = '<html><body>';
    $subject = utf8_decode('SWB Angebot Wärmepumpe');
